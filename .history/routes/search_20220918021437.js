...
router.get("/movie/:movieID", (req, res) => {
    ...
    Promise.all(endpoints.map((endpoint) => axios.get(endpoint))).then(([ {data: movies}, {data: similar} ]) => {
        
        console.log(movieDetails);

        res.render("similarMovies", {movies: movies, similar: similar});


    });
    // Promise.all([movieEndpoint, similarMovieEndpoint]).then([{}])
