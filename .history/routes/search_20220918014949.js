...
router.get("/movie/:movieID", (req, res) => {
    ...
    axios
        .get(similarMovieEndpoint)
        .then(similarResponse = (sim_response) => {
            const {similarMovieData} = sim_response;
            const allSimilarMovies = similarMovieData.results;
            const topTenSimilarMovies = allSimilarMovies.splice(0,10);
            //console.log(topTenSimilarMovies);
            // res.render("similarEndpoint", {similarMovies: topTenSimilarMovies})
            return topTenSimilarMovies;

        });

    axios
    .get(movieEndpoint)
    .then(movieResponse = (response) => {
        const {data} = response;
        const movieDetails = data;
        console.log(movieDetails);
        return movieDetails
    });
    res.render("similarMovies", {movies: movieResponse(), similarMovies: similarResponse()});
});
